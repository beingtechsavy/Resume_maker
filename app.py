...
class ResumeApp:
    ...
    def render_builder(self):
        ...
        # Domain/Career Path Selection
        categories = list(JOB_ROLES.keys())
        selected_category = st.selectbox("Select Your Career Domain", categories, help="Choose the domain that best matches your career path")
        
        # Get roles for selected category
        roles = list(JOB_ROLES[selected_category].keys()) if selected_category in JOB_ROLES else []
        selected_role = st.selectbox("Select Specific Role", roles, help="Choose a specific role to get tailored template recommendations")
        
        # Import domain templates
        from templates.domain_templates import get_domain_for_role, DOMAIN_TEMPLATE_STYLES, get_template_description
        
        # Get domain-specific templates
        domain_templates = DOMAIN_TEMPLATE_STYLES.get(selected_category.split(" ")[0], {})
        
        if domain_templates:
            # Template selection with domain-specific options
            template_options = list(domain_templates.keys())
            selected_template = st.selectbox("Select Resume Template", template_options)
            
            # Get template description
            template_description = get_template_description(selected_category, selected_template)
        ...
        st.markdown(f"""
            <div class="profile-section">
                ...
                <div class="social-links">
                    ...
                    <a href="https://www.linkedin.com/in/saumik-chakraborty-" class="social-link" target="_blank">
