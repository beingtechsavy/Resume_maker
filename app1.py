"""
SnapResume - Main Application
"""
import streamlit as st

# Set page config at the very beginning
st.set_page_config(
    page_title="SnapResume",
    page_icon="🚀",
    layout="wide"
)
...
if 'show_admin_login_modal' not in st.session_state:
...
class ResumeApp:
    def __init__(self):
        ...
        # Load 3D text effects CSS
        with open('style/3d-text-effects.css') as f:
            st.markdown(f'<style>{f.read()}</style>', unsafe_allow_html=True)
    ...
    def display_auth_buttons(self):
        """Display login and signup buttons in the top right corner"""
        # Create a container for the auth buttons
        auth_container = st.container()
        
        # Use custom CSS to position the container in the top right
        st.markdown("""
        <style>
        .auth-container {
            position: fixed;
        ...
        # Create HTML for the auth buttons
        if st.session_state.user_logged_in:
            ...
            <div class="auth-container">
                ...
                <button class="auth-button admin" onclick="window.location.href='?admin_login=true'">Admin</button>
            </div>
            """
        else:
            auth_html = """
            <div class="auth-container">
                <button class="auth-button" onclick="window.location.href='?login=true'">Login</button>
                <button class="auth-button signup" onclick="window.location.href='?signup=true'">Sign Up</button>
                <button class="auth-button admin" onclick="window.location.href='?admin_login=true'">Admin</button>
            </div>
            """
        ...
        elif "signup" in params:
            st.session_state.show_signup_modal = True
            st.query_params.clear()
        elif "admin_login" in params:
            st.session_state.show_admin_login_modal = True
            st.query_params.clear()
        elif "logout" in params:
            # Log the logout action
            if st.session_state.user_email:
                from config.database import log_user_action
                log_user_action(st.session_state.user_email, "logout")
        ...
        if st.session_state.show_login_modal:
            ...
            with login_modal:
                ...
                <div class="login-modal">
                    <div class="modal-content">
                        ...
                        <div class="form-footer">
                            Don't have an account? <a href="?signup=true">Sign up</a>
                        </div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
                
                # Handle form submission with Streamlit components
                with st.form(key="login_form", clear_on_submit=True):
                    ...
                    if submit_button:
                        from config.database import verify_user, log_user_action, get_user_by_email
                ...
                if st.button("Close", key="close_login_modal"):
                    st.session_state.show_login_modal = False
                    st.rerun()
    
    def display_signup_modal(self):
        """Display the signup modal"""
        if st.session_state.show_signup_modal:
            # Create a modal-like container
            signup_modal = st.container()
            
            with signup_modal:
                # Add a semi-transparent overlay (reusing the same CSS as login modal)
                st.markdown("""
                <div class="login-modal">
                    <div class="modal-content">
                        <div class="modal-header">
                        ...
                        <form id="signup-form">
                            ...
                            <div class="form-group">
                                ...
                                <input type="tel" class="form-input" id="signup-phone" placeholder="Enter your phone number">
                            </div>
                            <div class="form-group">
                                <label class="form-label">Password</label>
                ...
                # Close modal when clicking outside
                if st.button("Close", key="close_signup_modal"):
                    st.session_state.show_signup_modal = False
                    st.rerun()
    
    def display_admin_login_modal(self):
        """Display the admin login modal"""
        if st.session_state.show_admin_login_modal:
            # Create a modal-like container
            admin_login_modal = st.container()
            
            with admin_login_modal:
                # Add a semi-transparent overlay (reusing the same CSS as login modal)
                st.markdown("""
                <style>
                .admin-modal-content {
                ...
                <div class="login-modal">
                    <div class="admin-modal-content">
                        ...
                        <form id="admin-login-form">
                            ...
                            <button type="submit" class="admin-form-button">Login as Admin</button>
                        </form>
                    </div>
                </div>
                """, unsafe_allow_html=True)
                ...
                with st.form(key="admin_login_form", clear_on_submit=True):
                    ...
                    submit_button = st.form_submit_button(label="Login as Admin")
                    
                    if submit_button:
                        from config.database import verify_admin, log_admin_action
                ...
                # Close modal when clicking outside
                if st.button("Close", key="close_admin_login_modal"):
                    st.session_state.show_admin_login_modal = False
                    st.rerun()
    ...
    def render_home(self):
        ...
        # Display auth buttons in the top right corner
        self.display_auth_buttons()
        
        # Display login/signup modals if needed
        self.display_login_modal()
        self.display_signup_modal()
        self.display_admin_login_modal()
    ...
    def apply_global_styles(self):
        ...
        /* Main Header */
        .hero-title, .main-header h1 {
            font-size: 3em;
            font-weight: bold;
            background: linear-gradient(45deg, #1565C0 20%, #FB8C00 80%);
            -webkit-background-clip: text;
            background-clip: text;
            color: transparent;
            text-shadow: 0px 3px 6px rgba(0, 0, 0, 0.6), 1px 1px 0 #1565C0, -1px -1px 0 #FB8C00;
            letter-spacing: 2px;
        ...
        h1:hover, h2:hover, h3:hover {
            transform: translateZ(10px) rotateX(3deg) scale(1.03);
            text-shadow: 0px 4px 8px rgba(0, 0, 0, 0.8), 2px 2px 0 #1565C0, -2px -2px 0 #FB8C00;
            filter: drop-shadow(0 0 5px rgba(251, 140, 0, 0.5));
        }
        ...
        .feature-card, .stCard {
            ...
            box-shadow: 0 8px 24px rgba(21, 101, 192, 0.25) !important;
        ...
        .css-1d391kg, [data-testid="stSidebar"] {
            background-image: url('https://images.unsplash.com/photo-1506318137071-a8e063b4bec0?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1970&q=80');
            background-size: cover;
            background-position: center;
            background-repeat: no-repeat;
            background-attachment: fixed;
        ...
        /* Add a dark overlay to sidebar for better text visibility */
        ...
        .accent-element, .highlight-text, .stCheckbox label span p {
            color: #FB8C00 !important;
        }
        
        /* Feature Cards with Orange Accents */
        .feature-card i, .feature-card h3 {
            color: #FB8C00 !important;
            text-shadow: 0px 1px 2px rgba(0, 0, 0, 0.5);
        }
        
        /* Primary Action Buttons - Blue with Orange Hover */
        .primary-button button, button[data-testid="StyledFullScreenButton"] {
            background: linear-gradient(45deg, #1565C0, #0D47A1) !important;
            border: 2px solid #FB8C00 !important;
        }
        
        .primary-button button:hover, button[data-testid="StyledFullScreenButton"]:hover {
            background: linear-gradient(45deg, #FB8C00, #F57C00) !important;
            border-color: #1E88E5 !important;
        }
        
        /* Tabs with Blue/Orange Theme */
    ...
    def render_about(self):
        ...
        st.markdown("""
            <div class="hero-section">
                <h1 class="hero-title">About SnapResume</h1>
                <p class="hero-subtitle">A powerful AI-driven platform for optimizing your resume</p>
            </div>
        """, unsafe_allow_html=True)
        
        # Profile Section
        # Get Saumik's image path and convert to base64
        saumik_image_path = os.path.join(os.path.dirname(__file__), "assets", "Smart-AI-Resume-Analyzer\logo.jpg")
        saumik_image_base64 = None
        try:
            with open(saumik_image_path, "rb") as image_file:
                encoded = base64.b64encode(image_file.read()).decode()
                saumik_image_base64 = f"data:image/svg+xml;base64,{encoded}"
        except Exception as e:
            print(f"Error loading Saumik's image: {e}")
            
        st.markdown(f"""
            <div class="profile-section">
                <img src="{saumik_image_base64 if saumik_image_base64 else 'https://avatars.githubusercontent.com/Saumik17rkl'}" 
                     alt="Saumik Chakraborty" 
                     class="profile-image"
                     onerror="this.onerror=null; this.src='https://avatars.githubusercontent.com/Saumik17rkl';">
                <h2 class="profile-name">Saumik Chakraborty</h2>
                <p class="profile-title">AI/ML Enthusiast</p>
                <div class="social-links">
                    <a href="https://github.com/Saumik17rkl" class="social-link" target="_blank">
                        <i class="fab fa-github"></i>
                    </a>
                    <a href="https://www.linkedin.com/in/saumik-chakraborty-51a499257/" class="social-link" target="_blank">
                        <i class="fab fa-linkedin"></i>
                    </a>
                    <a href="mailto:saumik17rkl@gmail.com" class="social-link" target="_blank">
                        <i class="fas fa-envelope"></i>
                    </a>
                </div>
                <p class="bio-text">
                    Hello! I'm a passionate Full Stack Developer with expertise in AI and Machine Learning. 
                    I created SnapResume to revolutionize how job seekers approach their career journey. 
        ...
        # Remove the duplicate Saumik profile section
        # The code for the second profile section is removed as it's redundant
        
        # Get Saumik's image path and convert to base64
        saumik_image_path = os.path.join(os.path.dirname(__file__), "assets", "saumik_chakraborty.svg")
        saumik_image_base64 = None
        try:
            with open(saumik_image_path, "rb") as image_file:
                encoded = base64.b64encode(image_file.read()).decode()
        ...
        st.markdown("""
            <div class="vision-section">
                ...
                <p class="vision-text">
                    "SnapResume represents my vision of democratizing career advancement through technology. 
    ...
    def render_feedback_page(self):
        ...
        st.markdown("""
            <div class="feedback-header">
                ...
                <p>Help us improve SnapResume with your valuable feedback</p>
            </div>
        """, unsafe_allow_html=True)
    ...
    def main(self):
        ...
        # Display auth buttons in the top right corner on all pages
        self.display_auth_buttons()
